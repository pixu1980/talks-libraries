(function () {
      ...
      if (attributeFilter) {
        whenDefined(is).then(() => {
          ...
          for (let i = 0, { length } = attributeFilter; i < length; i++) {
            if (target.hasAttribute(attributeFilter[i])) dispatch({ target, attributeName: attributeFilter[i], oldValue: null });
          }
        });
      }
      return target;
  ...
  getOwnPropertyNames(self)
    ...
    .forEach((k) => {
      ...
      defineProperty((HTMLBuiltIn.prototype = HTMLElement.prototype), 'constructor', { value: HTMLElement });
